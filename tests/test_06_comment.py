...
class Test06CommentAPI:
    ...
    def test_02_comment(self, user_client, admin):
        ...
        for item in data['results']:
            if item.get('text') == 'qwerty':
                comment = item
        assert comment, 'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/comments/` ' \
    ...
    def test_04_comment_check_permission(self, client, user_client, admin):
        ...
        assert response.status_code == 401, \
            f'Проверьте, что при POST запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/comments/` ' \
            f'без токена авторизации возвращается статус 401'
        response = client.patch(f'{pre_url}{comments[1]["id"]}/', data=data)
        assert response.status_code == 401, \
            f'Проверьте, что при PATCH запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/comments/{{comment_id}}/` ' \
            f'без токена авторизации возвращается статус 401'
        response = client.delete(f'{pre_url}{comments[1]["id"]}/')
        assert response.status_code == 401, \
            f'Проверьте, что при DELETE запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/comments/{{comment_id}}/` ' \
            f'без токена авторизации возвращается статус 401'
